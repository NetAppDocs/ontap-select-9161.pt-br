...
Um cluster de dois nós com um mediador pode abranger dois data centers. Para mais informações, consulte a seção link:reference_plan_best_practices.html#two-node-stretched-ha-metrocluster-sds-best-practices["Práticas recomendadas de HA estendido de dois nós (MetroCluster SDS)"] .
...
* ONTAP Select agregado espelhado* image:DDHA_03.jpg["ONTAP Select agregado espelhado"]
