...
|===
. Depois que o novo servidor de gerenciamento for adicionado, você pode clicar opcionalmente image:icon_kebab.gif["Opções"] e selecione uma das seguintes opções:
+
...
Configure o PIN do YubiKey e gere ou importe a chave privada e o certificado do Remote Support Agent (RSA) ou do Elliptic Curve Digital Signature Algorithm (ECDSA) seguindo as etapas em link:https://docs.netapp.com/us-en/ontap-technical-reports/security.html#multifactor-authentication["TR-4647: Autenticação multifator no ONTAP"^] .
...
. Adicione a chave gerada ao ONTAP Select Implantar usando o  `security publickey add -key <key>` comando.
+
[listing]
----
(ONTAPdeploy) security publickey add -key "ssh-rsa <key> user@netapp.com"
----
. Habilitar autenticação MFA com o  `security multifactor authentication enable` comando.
+
[listing]
----
