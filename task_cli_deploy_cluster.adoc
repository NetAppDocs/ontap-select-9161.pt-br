...
.RAID de hardware
--
Se você utiliza um controlador RAID local, é necessário criar pelo menos um datastore (ESX) ou um link:kvm-host-configuration-and-preparation-checklist.html#create-storage-pool["pool de armazenamento (KVM)"] em cada nó para os dados do sistema, bem como para os agregados raiz e de dados. Você deve anexar o pool de storage como parte da configuração do nó ONTAP Select.

--
.RAID de software
--
Se você utiliza RAID por software, é necessário criar pelo menos um datastore (ESX) ou um link:kvm-host-configuration-and-preparation-checklist.html#create-storage-pool["pool de armazenamento (KVM)"] para os dados do sistema e garantir que as unidades SSD estejam disponíveis para os agregados raiz e de dados. Você deve conectar o pool de storage e os discos como parte da configuração do nó ONTAP Select.

--
====
. Versões ONTAP Select disponíveis
+
O utilitário de administração Deploy contém uma única versão do ONTAP Select. Se você deseja implantar clusters usando uma versão anterior do ONTAP Select, você deve primeiro link:task_cli_deploy_image_add.html["adicione a imagem ONTAP Select"] para sua instância de implantação.

. Licença ONTAP Select para uma implantação de produção
+
Antes de implantar um cluster ONTAP Select em um ambiente de produção, você deve adquirir uma licença de capacidade de armazenamento e baixar o arquivo de licença associado. Você pode link:concept_lic_production.html["licenciar o armazenamento em cada nó"] usando o modelo _Níveis de Capacidade_ ou licenciar um pool compartilhado usando o modelo _Pools de Capacidade_.
...
.Antes de começar
Execute o verificador de conectividade de rede usando o link:task_adm_connectivity.html["interface da web"] ou o link:task_cli_connectivity.html["CLI"] para confirmar a conectividade entre os nós do cluster na rede interna.
